...
test('иконки списка упражнений объявлены, даже если в прогон не попали', () => {
    // Прогон выше видит только то, что доезжает до DOM, а упражнения одного
    // курса не показываются в другом: иконки еврейских видов при греческих
    // данных не встретятся ни разу. Их надо сверять со списком напрямую.
    const declared = declaredIcons();
    const app = loadApp();
    const icons = app.get('LESSON_DRILL_GROUPS.flatMap(g => g.drills.map(d => d.icon)).join(",")').split(',');
    app.close();

    const missing = icons.filter(n => !declared.has(n));
    assert.deepStrictEqual(missing, [],
        'иконка упражнения не объявлена в icon_names: ' + missing.join(', '));
});
